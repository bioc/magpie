...
# Quickstart
PAM offers the function ``QuickPower()`` for users who need quick power calculation results. This function takes seconds to run and uses pre-calculated results from three publicly available MeRIP-seq datasets. The details of these datasets can be found in \@ref(QuickPower)). To use this function and save and plot the results, simply run the following lines of code:
