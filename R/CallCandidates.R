# Call candidates regions with bin data
CallCandidates <- function(Counts, bins,
    ...
    if (nrow(thispeak) >= 1) {
        ...
        bins.GR <- GRanges(Rle(bins$chr), IRanges(
            bins$start,
            bins$end
        ), Rle(bins$strand))
        thiscount <- getPeakCounts(
            peaks = thispeak,
            allCounts = Counts,
            allBins = bins.GR
        )
        ...
        ##########
        Candidates <- list(
            Regions = thispeak, Counts = thiscount,
            lg.fc = smooth.lfc,
            sf = sf
        )
        if (nrow(thiscount) >= 2) {
        ...
        Candidates <- list()
        message("No candidates from any samples!", sep = "\n")
    }
    return(Candidates)
}
