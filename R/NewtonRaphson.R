...
dL.phi <- function(x, y, sx, sy, D, R, phi, theta) {
    ...
    tmp1 <- phi^
        {
            -2
        } * sum((1 - mu) * log(1 + theta * sx)) + phi^
            {
                -2
            } * sum(mu * log(1 + theta * sy))
    tmp2 <- -phi^{
        -2
    } * sum((1 - mu) * digamma((1 - mu) * (phi^
        {
            -1
        } - 1) + x))
    tmp3 <- phi^
...
d2L.phi2 <- function(x, y, sx, sy, D, R, phi, theta) {
    ...
    tmp1 <- -2 * phi^{
        -3
    } * sum((1 - mu) * log(1 + theta * sx) +
        mu * log(1 + theta * sy))
    tmp2 <- sum(trigamma((1 - mu) * (phi^
        {
            -1
        } - 1) + x) * (phi^
        {
...
E.d2L.theta2 <- function(x, y, sx, sy, D, R, phi, theta,
    ...
    for (j in seq_len(length(x))) {
        tmp1 <- tmp1 + sum(fx(
            this.x = 0:M, this.mu = mu[j],
            this.sx = sx[j], phi = phi, theta = theta
        )) + sum(fy(
            this.y = 0:M, this.mu = mu[j],
        ...
        for (l in setdiff(seq_len(length(y)), seq_len(j - 1))) {
            if (l == j) {
                ...
                E_ttT[j, l] <- (c[j] + sum(
                    ...
                    this.y = 0:M, this.mu = mu[l],
                    this.sy = sy[l], phi = phi, theta = theta
                )) -
                    sum(fx.1(
                        this.x = 0:M, this.mu = mu[l],
...
d2L.Rtheta <- function(x, y, sx, sy, D, R, phi, theta) {
    ...
    (phi^
        {
            -1
        } - 1) * colSums(diag(as.vector(sx / (1 + theta * sx) -
